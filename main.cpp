...
struct GroceryStore {
  ...
  struct Supplier // Suppliers that deliver different products to the store
  {
    ...
    void orderFilled(int orderNumber, float orderAmount, float dateDelivered,
                     bool itemsMissing); // post-order delivery data
    void placeNewOrder(int orderNumber,
                       float orderAmount); // place a new order with supplier
  };
  // this is the end of the nested class
  ...
  //    - charge customer  //NOTE: function parameter is relevant to the amount
  //    of groceries purchased plus taxes
...
struct Airplane {
  ...
  struct Passenger {
    ...
    void dietRestrictions(bool nonDairy = false, bool nutAllergy = false,
                          bool vegetarian = false);
    float getMilesTraveledAnnually(int customerID); // returns the number of miles
                                              // customer traveled previous year
  };
...
struct Laptop {
  ...
  // 3 things it can do:
  //     - play audio
  bool playAudio(std::string audioFileName); // returns true if file is found
                                             // and is the correct format
  //    - play video
  bool playVideo(std::string videoFileName); // returns true if file is found
                                             // and is the correct format
  //    - run an application
  int runApp(std::string applicationName); // returns the amount of memory used
                                           // by the application
};
